...
func NewVoucherRetryWorker(cfg *VoucherConfig, store *VoucherTransmissionStore, pushService *VoucherPushService) *VoucherRetryWorker {
	...
	if cfg != nil {
		...
		if cfg.RetryWorker.MaxAttempts > 0 {
			batchSize = cfg.RetryWorker.MaxAttempts // reuse as reasonable concurrency if no dedicated setting
		}
	}
	if batchSize < 5 {
		batchSize = 5
