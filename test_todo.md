...
## Voucher Transmission & Push Checklist
...
- [ ] `--voucher-list` formatting: verify columns and filters (status, GUID, limit).
- [ ] `--voucher-retransmit-id`: ensure resolver rehydrates and errors bubble up.
- [ ] `--voucher-purge-id`: confirm file deletion + DB row removal behavior.
...
- [ ] CLI smoke test suite (Go test or bash) to exercise commands against a temp DB.
- [ ] e2e flow: voucher creation → transmission row → retry worker success → CLI purge.

## Lint/Test Debt

- [ ] Remove or gate unused logging helpers in `main.go` (debugDisabledHandler/noopHandler) or reference them in tests.
- [ ] Remove dead helpers in `voucher_signing_service.go` (`encode*PEM`, `generateOwnerKey`, `parseKeyType`).
- [ ] Delete/replace unused `runTests()` helper in `test_generic_handlers.go`.
- [ ] Address remaining `errcheck` findings from `golangci-lint` (14 spots) after unused helpers resolved.
- [ ] Run `shellcheck -s sh tests/*.sh` once Go linters are clean.
