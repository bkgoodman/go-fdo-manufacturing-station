...
func NewVoucherFileStore(cfg *VoucherConfig) *VoucherFileStore {
	var dir string
	if cfg != nil {
		dir = strings.TrimSpace(cfg.VoucherFiles.Directory)
		if dir == "" {
			dir = strings.TrimSpace(cfg.SaveToDisk.Directory)
		}
	}

	if dir == "" {
		dir = filepath.Join("data", "vouchers")
	}

	return &VoucherFileStore{directory: dir}
}
